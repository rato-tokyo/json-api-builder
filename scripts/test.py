...
def main():
    ...
    # 1. 依存関係のインストール
    if not run_command("python -m pip install -e .[test]", "テスト依存関係のインストール"):
        return False
    ...
    # 3. カバレッジ付きテスト実行
    print("\n📊 カバレッジ付きテスト実行")
    if not run_command("python -m pytest tests/ --cov=src/json_api_builder --cov-report=term-missing", "カバレッジテスト"):
